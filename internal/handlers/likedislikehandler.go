...
func LikeDislikeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		if err != nil {
			ErrorHandler(w, http.StatusInternalServerError, "Failed to get like/dislike info, Please try again later.", "")
			return
		}
		...
		if u.Count > 0 {
			...
			if err != nil {
				ErrorHandler(w, http.StatusInternalServerError, "Failed to update like/dislike, Please try again later.", "")
				return
			}
		} else {
			...
			if err != nil {
				ErrorHandler(w, http.StatusInternalServerError, "Like Or Dislike failed, Please try again later.", "")
				return
			}
